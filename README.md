...
2. **สร้างบัญชีแอดมิน** — Authentication > Users > Add user ใส่อีเมลและรหัสผ่าน
3. **ให้สิทธิ์แอดมิน** — รันใน SQL Editor
   ```sql
...
4. **ใส่ anon key** — เปิด `js/config.js` แล้วใส่ค่า anon public key จาก Project Settings > API (ห้ามใช้ service_role key)
5. **Deploy ขึ้น Netlify** — Add new site > Import from GitHub > เลือก repo นี้ ไม่ต้องตั้ง build command (ใช้ `netlify.toml` ที่มีอยู่)
...
- ถ้าลูกค้าหมุนเองจากมือถือ แนะนำโหมด LINE ด้านบน ถ้าไม่ใช้โหมด LINE อย่างน้อยควรเปิด "1 เครื่องหมุนได้ 1 ครั้งต่อวัน" (ข้อจำกัด: ลูกค้าล้างข้อมูล browser หรือเปลี่ยนเครื่องแล้วหมุนใหม่ได้)
